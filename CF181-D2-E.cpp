/*
Observe that if absolute difference of either x or y coordinate is greater than 4 then second
player can always restrict the first player from winning.
Remaining has to be done either by brute/pen-paper
*/
...
int main(){
   	...
   	int n, m, x1, x2, y1, y2;
   	cin >> n >> m >> x1 >> y1 >> x2 >> y2;
   	if(abs(x1 - x2) > 4 || abs(y1 - y2) > 4 || abs(x1 - x2) + abs(y1 - y2) >= 7)cout << "Second" << endl;
